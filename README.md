...
- 現在の到達点: `3.1.0.0 r195`。条件・発動タグを全原文の明示マーカーから生成し、レアリティ違いを混同せず、現在の検索カテゴリに該当武将が存在するタグだけを表示します。武将検索で実際に選択可能な939タグを1件ずつ画面投影まで照合する総点検を追加し、技能レベル単一選択など既存機能も維持しています。
