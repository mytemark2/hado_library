...
assert(versionSource.includes('revision: 178'), 'preview revision must be r178');
...
assert.strictEqual(preview.context.window.HADO_APP_DISPLAY_VERSION, '3.1.0.0 Update05 r178', 'preview must display release version, Update, and revision');
assert.strictEqual(preview.nodes.get('h1').textContent, '覇道ライブラリ 3.1.0.0 Update05 r178', 'preview heading must include the active Update');
...
for (const asset of ['hado_update04.css', 'hado_update05.css', 'hado_condition_model.js', 'hado_formation_condition_evaluator.js', 'hado_detail_condition_presenter.js', 'hado_version.js', 'hado_core.js', 'hado_formation.js', 'hado_search.js', 'hado_bootstrap.js']) {
  assert(indexHtml.includes(`${asset}?v=05-r178`), `${asset} must use the active Update cache key`);
}
...
console.log('3.1.0.0 Update05 version policy regression ok');
