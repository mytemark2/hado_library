...
js=js.replace("ally_defense_buff:['味方防御上昇','味方部隊の防御'],combat_start_tactic_gauge:['交戦開始時戦法ゲージ']", "ally_defense_buff:['味方防御上昇','味方部隊の防御'],combat_start_tactic_gauge:['交戦開始時戦法ゲージ'],self_disadvantage_countermeasure:['自部隊不利対策','自部隊耐性強化','弱化無効','弱化解除','状態変化無効','回避','無視','耐性'],ally_non_damage_effect:['味方非ダメージ効果','味方部隊','有利変化','強化効果','弱化解除','状態変化無効','負傷兵回復','防御','耐性']")
...
required=['Update04.2','self_disadvantage_countermeasure','ally_non_damage_effect','function roleRows(role)','v._s.matched.length>0','roleRows(id).length','参考一致数が0件の候補は表示しません。','function chipLabels(']
...
info.update({'releaseStatus':'development','releaseVersion':'3.0.0.0','updateNo':'04','revision':2,'displayVersion':'3.0.0.0 Update04.2','summary':'Update04.2: add vaccine type for self disadvantage countermeasures and ally non-damage effects; exclude zero-match candidates and align role tab counts with visible candidates.','updatedAt':datetime.now(timezone(timedelta(hours=9))).isoformat(timespec='seconds')})
info_path.write_text(json.dumps(info,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
report={'version':'3.0.0.0','update':'Update04.2','typeCount':len(score['types']),'vaccineType':True,'zeroMatchExcluded':True,'roleTabCountUsesVisibleRows':True,'chipObjectRenderingFixed':True,'candidateDisplayOnly':True,'indexSha256':hashlib.sha256(index).hexdigest()}
