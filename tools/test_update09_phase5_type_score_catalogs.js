...
const requiredChangeItems = ['attack_up','defense_up','intelligence_up','tactic_power_up','normal_attack_power_up','anti_object_up','critical_rate_up','critical_power_up','critical_tactic_rate_up','critical_tactic_power_up','attack_speed_up','normal_attack_target_count_up','range_up','tactic_speed','tactic_reduction','initial_tactic_gauge','combat_start_tactic_gauge','tactic_gauge','chain_rate','damage_reduction','attribute_resistance','healing','wounded_recovery','wounded_survival','annihilation_avoidance','remaining_troops','weakening_nullify','weakening_remove','weakening_avoid','status_nullify','status_remove','severance_counter','isolation_counter','chain_nullify_counter','control_counter','buff_protection','enemy_attack_down','enemy_defense_down','enemy_anti_object_down','tactic_delay','chain_nullify_to_enemy','confuse','fear','severance','isolation'];
...
const byType = new Map(types.map(t => [t, table.filter(r => r.typeId === t)]));
types.forEach(typeId => { const rows = byType.get(typeId); assert(rows.length > 0, `missing type: ${typeId}`); assert(rows.some(r => r.scoreRole === 'P'), `${typeId} has no Primary row`); assert(!rows.every(r => r.scoreRole === 'S'), `${typeId} is Support-only`); rows.forEach(r => assert(r.dedupePolicy && r.dedupePolicy.includes('evidenceGroupKey'), `${typeId} missing evidenceGroupKey dedupe policy`)); });
...
assert(debuffPrimary.dependency.includes('Supportだけで高評価にしない'), 'debuff_interference missing support cap dependency');
const attackSpeed = byType.get('attack_speed');
const tacticSpeed = byType.get('tactic_speed');
assert(attackSpeed.some(r => r.scoreRole === 'X' && r.denyChangeItems.includes('tactic_speed')), 'attack_speed must deny tactic_speed');
assert(tacticSpeed.some(r => r.scoreRole === 'X' && r.denyChangeItems.includes('attack_speed_up')), 'tactic_speed must deny attack_speed_up');
assert(byType.get('critical_normal').some(r => r.scoreRole === 'X' && r.denyChangeItems.includes('critical_tactic_rate_up')), 'critical_normal must deny tactic critical');
assert(byType.get('critical_tactic').some(r => r.scoreRole === 'X' && r.denyChangeItems.includes('critical_rate_up')), 'critical_tactic must deny normal critical');
