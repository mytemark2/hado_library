...
assert.ok(diagnostic.sourceMarkerBlockCount >= diagnostic.sourceTagItemCount, 'source markers must retain non-vacuous coverage');
...
assert.ok(engagementOwners.length > 0, 'engagement-start search regression must retain current source examples');
...
assert(indexHtml.includes('hado_search_clause_integration.js?v=3.1.1.0-r205'));
assert(!indexHtml.includes('06-r180'));
assert(!indexHtml.includes('07-r179'));
assert(versionSource.includes("updateNo: ''"));
assert(versionSource.includes('revision: 205'));
...
assert.strictEqual(context.window.HADO_APP_DISPLAY_VERSION, '3.1.1.0 r205');
