...
req('候補に追加（${pickedCount}件）' in tc,'candidate workspace edit mode must show selected count')
req(tc.find('候補に追加（${pickedCount}件）') < tc.find('この型で新規部隊'),'candidate add action must appear before new formation')
