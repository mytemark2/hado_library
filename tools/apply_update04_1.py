...
for token in required:
  if token not in js:
    raise SystemExit(f'Update04.1 JS audit failed: missing {token}')
for forbidden in ['evaluationTypeId=', 'candidateTray=', "localStorage.setItem('hado.formation", 'autoPlace']:
  if forbidden in js:
    raise SystemExit(f'Update04.1 safety audit failed: forbidden token {forbidden}')
...
info.update({
  ...
  'updateNo':'04',
  'revision':1,
  'displayVersion':'3.0.0.0 Update04.1',
  'summary':'Update04.1: add selected-type role candidate browser for nine roles. Candidate display only; no auto placement and no formation save mutation.',
  'updatedAt':updated
})
...
report={
  'version':'3.0.0.0',
  'update':'Update04.1',
  'scope':'selected-type role candidate browser only',
  'roleCount':9,
  'roles':['main_general','vice_general','support_general','attendant','equipment','formation','siege_weapon','warhorse','warhorse_skill'],
  'safety':{
  ...
  'regressionChecks':['nine role tabs','selected type display','same-type reference match count','query filter','candidate-only note','HTML equality'],
  'indexSha256':hashlib.sha256(index).hexdigest()
}
rp=ROOT/'report/HADO_APP_3.0.0.0_UPDATE04.1_REPORT.json'; rp.parent.mkdir(exist_ok=True)
