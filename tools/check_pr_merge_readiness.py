...
def main() -> int:
    ...
    parser.add_argument("--base", default="feature/app-3.1.2.0", help="Canonical base branch for application PRs")
    args = parser.parse_args()

    require_clean_worktree()
    ensure_remote(args.remote, args.remote_url)
    remote_ref = f"refs/remotes/{args.remote}/{args.base}"
    source_ref = f"refs/heads/{args.base}"
    run(["git", "fetch", args.remote, f"+{source_ref}:{remote_ref}", "--prune"])
