...
assert(status.includes("option.addEventListener('pointerdown',e=>e.preventDefault())"), 'touch selection must retain the input/list relationship until click commits');
assert(status.includes("option.addEventListener('click',()=>selectTagCandidate(tag,'candidate-tap'))"), 'a candidate tap must select the canonical tag');
