...
assert(versionSource.includes('revision: 169'), 'preview revision must be r169');
...
assert.strictEqual(preview.context.window.HADO_APP_DISPLAY_VERSION, '3.0.2.0 r169', 'preview must display release version and revision without Update');
assert.strictEqual(preview.nodes.get('h1').textContent, '覇道ライブラリ 3.0.2.0 r169', 'preview heading must omit Update');
...
for (const asset of ['hado_styles.css', 'hado_version.js', 'hado_core.js', 'hado_search.js']) {
  assert(indexHtml.includes(`${asset}?v=3.0.2.0-r169`), `${asset} must use the no-Update cache key`);
}
