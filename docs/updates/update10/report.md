...
- 正式公開Pages: PR [#264](https://github.com/mytemark2/hado_library/pull/264) により、`main` のランタイム更新だけで起動する `Deploy Hado Library Production Pages` workflow を追加した。merge commit `c6dbe8cf1b398af9c57648ffbe267bc9dbc3f40f` のデプロイ run `29718544655` は成功した。GitHub Pages設定は `build_type=workflow`、source=`main`、path=`/` へ切り替え済みである。デプロイ前に `index.html`、外部JavaScript/CSS、主要JSONを確認し、workflow契約は全体回帰124件へ常設した。
