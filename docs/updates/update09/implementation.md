...
### Update09.5.60 — 通常・状態変化・型検索の状態独立化

- `state.searchModeContexts` に通常・状態変化・型検索の3コンテキストを置き、キーワード、名称限定、タグ、対象カテゴリ、状態変化条件、型条件、選択結果、詳細タブをモード別に保存・復元する。
- 状態変化検索を離れる時は非同期処理だけを中止し、選択条件と結果cacheはコンテキストへ保持する。戻った時は保持結果を表示後、現行データ範囲で非同期検索を更新する。
- 通常検索以外では通常検索キーワードをruntime入力から外し、完全一致による自動選択も通常検索だけに限定する。
- 操作履歴へ現在モードの直列化可能なコンテキストを保存し、履歴復元でも同じ復元関数を使う。状態変化の非同期再取得中は選択詳細を消さない。
- `tools/test_update09_5_60_search_mode_isolation.js` を常設し、3モード条件・選択復元、非表示キーワード境界、履歴連携を検証する。
- JSON契約、Crawler出力、保存データ形式は変更しないため、Crawler再実行とJSON再生成は不要である。
- 可視版は `3.0.0.0 Update09.5.60 r150`。ロジックは既存の外部JavaScriptへ実装し、HTMLはruntime cache keyだけを更新する。

### Update09.5.61 — 編成判断サマリー・候補ワークスペース統合

- 分類: 編成判断UIの情報優先度不一致、および型候補選定と配置候補管理の画面分断。
- 根本原因: 結果サマリーは状態変化率の先頭行を件数制限して表示しており、部隊判断に必要な項目が固定されていなかった。型候補一覧と候補トレイは同じ `candidateTray` を扱うのに別モーダル・別起動ボタンを持ち、選定から配置までに画面遷移が必要だった。
- 恒久対策: 結果サマリー専用の6項目定義と集計関数を追加し、最終値ではなく編成由来の差分を表示する。候補UIは `hado_type_candidates.js` の単一ワークスペースへ統合し、`hado_candidate_tray.js` は候補モードを開くランチャーだけに限定する。
- 兵力は固定値と割合を別表示する。被ダメージは増減率、通常攻撃は攻撃速度と会心発生率、戦法初動は出陣時戦法ゲージ、戦法速度は加算率、戦法最大倍率は既存の戦法攻撃計算結果を表示する。
- 通常時・出陣時・戦法発動時・駐屯防衛時を保持し、条件文がある効果は「条件付き」として分離する。条件と固定/割合を暗黙に合算しない。
- 編集モードは型候補の複数選択と一括追加、候補モードは保存済み候補だけを役割別に表示し、カードから直接配置・削除する。型編成ナビからは必ず編集モード、部隊編成の固定ボタンからは候補モードで開く。
- 編集モードは全タブを正確な数値件数で初回表示する契約を維持しつつ、採点バッチを24件から120件へ拡大した。フレームへ制御を返す進捗描画は残し、統合後の初回表示待ちを短縮する。
- `candidateTray` の保存・sanitize・Export/Import、追加・削除・全削除・snapshot、役割別配置先、侍従UR以下ゲート、最終成立判定は `hado_formation.js` の既存経路を変更しない。
...
## Update09 完了記録（2026-07-20）

- Update09の最終正本は `fba2f1999f8f821002931ff2413f4f43cae0983f`。最終表示版は `3.0.0.0 Update09.5.65 r155`。
- 実装範囲はPC/スマホUI/UX、検索対象境界、状態変化検索、型検索、候補ワークスペース、保存データ切替、結果サマリー、関連リンク・装備検索索引を含む。
- 最終修正では候補ワークスペースの作成前draftをschema 2で保存し、型編成ナビ、通常ランチャー、外部イベント、ページ再読込の各経路で一致するdraftを復元するようにした。
- 全アプリ検証119項目、PR #246、App Validation、Preview同期、公開PreviewのPC/390x844実操作、Debug Logを確認済み。詳細は `report.md` を正本とする。
