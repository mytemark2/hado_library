...
### 変更概要
- 可視バージョンを `3.0.0.0 Update09.3.9` / revision 42 へ更新した。
...
- Moved the result summary strip into the same right-side/mobile stack immediately after the total score panel, so mobile formation edit order is now score panel -> result summary -> warhorse panel.
