...
- `3.0.0.0 Update08.3` として、実行時の表示バージョン定義を `hado_update_meta.js` の `HADO_VERSION` に集約し、型候補一覧は `window.HADO_APP_DISPLAY_VERSION` / `window.HADO_APP_VERSION_META` を参照するようにした。
