...
### Update08.22 - 保存データ型候補診断ログとAuto Merge設定失敗の可視化
- 保存データ＋限定の不具合原因を次回ログだけで追跡できるよう、型候補一覧の保存データ経路へ診断ログを追加した。
- `typeCandidate:saved-role-rows` で、ロール別の元候補数・保存所有通過数・スコア表示数・最終表示数・限定行数・ロール不一致行数・保存技能/技能Lv不足で除外された行数とサンプルを記録する。
- `window.HadoTypeCandidatesDebug.getDiagnostics()` から直近診断を確認できるようにし、`rowSkillRejection` / `auditSavedCandidateRows` をDebug APIへ公開した。
- Auto Merge workflowにリポジトリ設定診断を追加し、`Allow auto-merge` が無効、またはGraphQLのAuto Merge有効化が失敗した場合は警告で握りつぶさずworkflowを失敗させる。

### Update08.23 - 型候補一覧描画キャッシュと重い診断の非同期化
- 型候補一覧のロール別結果を `typeId / purposeId / role / query / savedSearchCacheSeq / 保存データ内容` でキャッシュし、タブ件数表示や選択状態変更で全候補を再スコアリングしないようにした。
- 候補一覧の一括スコアリング中は `HADO_TYPE_SCORE_TRACE_SUSPENDED` で `HadoTypeScore.recordTrace` を抑止し、候補ごとの診断オブジェクト更新を通常描画から外した。
- 保存データ型候補の診断ログは `requestIdleCallback` / `setTimeout` で非同期flushし、Console出力は `window.HADO_TYPE_CANDIDATE_VERBOSE_LOGS` が有効な場合だけに限定した。
- 保存技能除外の監査と実フィルタを1パス化し、同じ候補行に対する `rowSkillRejection` の二重実行を避けた。
- 部隊編成の `buildTacticAttackDiagnosticSnapshot()` は通常描画中に全武将・全戦法を同期走査しないよう、`deferTacticAttackDiagnosticSnapshot()` でidle時に遅延実行する。
