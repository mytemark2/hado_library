...
`3.1.0.0 Update06 r183`として完了した。r180で検索統合、r181で利用者向け表示修正、r182で「白眉」のような参照付与技能を通常技能と同じ条件・効果カードへ統合し、r183でデータ読込完了前に検索入力した際の未定義関数エラーを修正した。参照関係200件の全件回帰、自動Preview同期、読込中検索を含む公開PreviewのPC・スマートフォン実操作が合格した。全Update完了まで正式公開は行わない。
