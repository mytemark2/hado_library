...
`3.1.0.0 Update06 r187`としてWeb JSONの起動方式を改修する。r186の復旧内容・順序・検索・編成ロジックを維持し、通常起動ではブラウザ内の検証済みJSONを使用する。軽量マニフェストで更新を検出した場合だけ公開JSON一式を再取得し、通信失敗時は従来キャッシュで起動する。全Update完了まで正式公開は行わない。
