...
`3.1.0.0 Update06 r182`として完了した。r180で検索統合、r181で利用者向け表示修正、r182で「白眉」のように別技能から付与される技能も通常技能と同じ条件・効果カードへ統合した。付与されたLvだけを表示し、参照関係200件の全件回帰、自動Preview同期、公開PreviewのPC・スマートフォン実操作が合格した。全Update完了まで正式公開は行わない。
