...
`3.1.0.0 Update06 r184`として完了。r180で検索統合、r181で利用者向け表示修正、r182で「白眉」のような参照付与技能を通常技能と同じ条件・効果カードへ統合し、r183でデータ読込完了前検索を安定化した。r184では技能・検索・データ管理・編成画面の重複説明を削減し、必要説明をガイド、ツールチップ、アクセシビリティ名へ集約した。145項目のApp Validation、自動Preview同期、公開PreviewのPC・スマートフォン実操作が合格した。全Update完了まで正式公開は行わない。
