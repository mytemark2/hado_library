...
開始準備中。Preview同期元を3.1正本へ切り替え、全件条件センサスへ着手できる状態を整備する。
...
- [ ] 未走査技能が0件。
- [ ] 関連状態変化の未走査が0件。
- [ ] 現行condition blocksを持たない戦法/技能を個別確認した。
- [ ] 未確認残差が0件。
- [ ] 条件型/trigger/context/modifier/limit/reset/suppressionの一覧を作成した。
- [ ] 各条件型に代表例がある。
- [ ] gold setを30〜50件以上作成した。
- [ ] 現行condition blocksの再利用可否と不足点を明文化した。
- [ ] Update02でschemaを確定するための入力が揃った。
