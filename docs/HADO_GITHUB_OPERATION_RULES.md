...
`hado_app.js` の legacy monolithic artifact は削除済み。通常の機能修正・Phase更新・ガイド文言更新では、実際に読み込まれる `hado_core.js`、`hado_formation.js`、`hado_search.js` などの分割済み外部JSを編集する。削除済みの `hado_app.js` を再作成して実行時差分の置き場にしない。
