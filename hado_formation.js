...
function calculateFormationAutoScores(f,data={}){let diag;try{diag=calculateFormationTypeScore(f,data);}catch(err){debugLog('formationScore:empty',{formationId:f?.id||'',formationName:f?.name||'',reason:'score calculation error',message:err?.message||String(err)});return createFormationFallbackAutoScores(f,data,`score calculation error: ${err?.message||String(err)}`);}const selected=diag.candidateScores?.[0]||null;const scoreRows=(selected?.rows||[]).slice(0,5).map(row=>({label:row.label,score:row.score,unit:'件',matchedEffects:Array.isArray(row.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row.matchedParameters)?row.matchedParameters:[]}));const totalScore=selected?Number(selected.totalScore||0):0;return {totalScore,evaluationScore:totalScore,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount:Array.isArray(data?.skills)?data.skills.length:0,parameterCount:diag.effectSourceCount||diag.parameterRowCount||0,evaluationTypeName:selected?.typeName||formationEvaluationTypeDisplayName(f),scorePolicy:diag.selectedTypeId?'formation-effect-score-selected-type':'formation-effect-score-candidates',memberCount:0,candidateScores:diag.candidateScores||[],emptyReason:diag.emptyReason||''}};}
