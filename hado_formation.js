...
function openFormationGroupNameDialog(){state.formationGroupNameDialogOpen=true;renderFormationScreen();}
function closeFormationGroupNameDialog(){state.formationGroupNameDialogOpen=false;renderFormationScreen();}
...
function formationScoreRowToDebug(row){return {featureId:row?.featureId||'',label:row?.label||row?.statusEffectName||'',sourceKind:row?.sourceKind||'',sourceLabel:row?.sourceLabel||'',key:row?.key||'',group:row?.group||'',timing:row?.timing||'',value:row?.value||'',condition:row?.condition||'',matchedText:String(row?.matchedText||'').slice(0,240),rawText:String(row?.rawText||row?.matchedText||'').slice(0,500)};}

function formationScoreDetailPoint(row,metric){return 1;}
function formationScoreDetailFromMatchedRow(row,metric,index){const type=row?.sourceKind||'effect';const point=formationScoreDetailPoint(row,metric);const label=row?.label||row?.key||row?.statusEffectName||metric?.label||'一致項目';const rawText=String(row?.rawText||row?.matchedText||rowText?.(row)||'');return {label,point,source:row?.sourceLabel||(type==='parameter'?'変化率集計':'スコア根拠'),condition:formationScoreEvidenceCondition(row),value:row?.value||'',matchedText:String(row?.matchedText||rawText).slice(0,240),rawText:rawText.slice(0,500),evidenceType:type,reason:`${metric?.method||'matched_item_count'}: ${metric?.label||metric?.metricKey||'評価項目'} に一致した根拠`,featureId:row?.featureId||'',key:row?.key||'',index};}
...
function buildFormationEffectScoreEntity(f,data={}){const parameterRows=collectFormationAllParameterRows(data);const effects=Array.isArray(data?.effects)?data.effects:[];const parameterFeatures=parameterRows.map(r=>({featureId:`parameter:${norm(r.key||r.label||'')}`,label:r.label||r.key||'',statusEffectName:r.label||r.key||'',matchedText:[r.label||'',r.key||'',r.timingLabel||'',r.group||'',r.value||''].filter(Boolean).join(' '),sourceKind:'parameter',key:r.key||'',group:r.group||'',timing:r.timing||'',value:r.value||'',condition:r.condition||''}));const statusRefs=effects.map(e=>({featureId:`status_effect:${norm(e?.key||'')}`,label:e?.key||'',statusEffectName:e?.key||'',matchedText:[e?.key||'',e?.group||'',e?.timing||'',e?.rawText||'',e?.condition||'',e?.sourceLabel||'',effectSignedValue(e)].filter(Boolean).join(' '),sourceKind:'effect',sourceLabel:e?.sourceLabel||'',key:e?.key||'',group:e?.group||'',timing:e?.timing||'',value:effectSignedValue(e),condition:e?.condition||''}));return {roleId:'formation_effects',displayName:f?.name||'部隊',typeFeatures:parameterFeatures,statusEffectRefs:statusRefs,parameterRows,effects};}
function summarizeFormationRuleScore(rule,scoreResult){const rows=(scoreResult?.breakdown||[]).slice(0,5).map(row=>{const matched=Array.isArray(row?.rows)?row.rows:[];const scoreDetails=buildFormationScoreDetails(row);const detailTotal=sumFormationScoreDetails(scoreDetails);const score=Number(row?.itemCount||row?.conditionalMaxValue||detailTotal||0);if(scoreDetails.length&&detailTotal!==score)debugLog('formationScore:score-detail-mismatch',{label:row?.label||row?.metricKey||'',score,detailTotal,detailCount:scoreDetails.length});return {label:row?.label||row?.metricKey||'',score,scoreDetails,evidenceRows:scoreDetails,matchedEffects:matched.filter(v=>v?.sourceKind==='effect').map(formationScoreRowToDebug).slice(0,40),matchedParameters:matched.filter(v=>v?.sourceKind==='parameter').map(formationScoreRowToDebug).slice(0,40)};});return {typeId:rule?.typeId||'',typeName:rule?.typeName||rule?.typeId||'',totalScore:rows.reduce((sum,row)=>sum+Number(row.score||0),0),rows};}
...
function calculateFormationAutoScores(f,data={}){let diag;try{diag=calculateFormationTypeScore(f,data);}catch(err){debugLog('formationScore:empty',{formationId:f?.id||'',formationName:f?.name||'',reason:'score calculation error',message:err?.message||String(err)});return createFormationFallbackAutoScores(f,data,`score calculation error: ${err?.message||String(err)}`);}const selected=diag.candidateScores?.[0]||null;const scoreRows=(selected?.rows||[]).slice(0,5).map(row=>{const details=Array.isArray(row.scoreDetails)?row.scoreDetails:(Array.isArray(row.evidenceRows)?row.evidenceRows:[]);const detailTotal=sumFormationScoreDetails(details);if(details.length&&detailTotal!==Number(row.score||0))debugLog('formationScore:score-detail-mismatch',{label:row.label||'',score:Number(row.score||0),detailTotal,detailCount:details.length});return {label:row.label,score:row.score,unit:'',scoreDetails:details,evidenceRows:details,matchedEffects:Array.isArray(row.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row.matchedParameters)?row.matchedParameters:[]};});const totalScore=selected?Number(selected.totalScore||0):0;return {totalScore,evaluationScore:totalScore,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount:Array.isArray(data?.skills)?data.skills.length:0,parameterCount:diag.effectSourceCount||diag.parameterRowCount||0,evaluationTypeName:selected?.typeName||formationEvaluationTypeDisplayName(f),scorePolicy:diag.selectedTypeId?'formation-effect-score-selected-type':'formation-effect-score-candidates',memberCount:0,candidateScores:diag.candidateScores||[],emptyReason:diag.emptyReason||''}};}
...
function normalizeFormationScoreEvidenceRows(row){const provided=Array.isArray(row?.scoreDetails)?row.scoreDetails:(Array.isArray(row?.evidenceRows)?row.evidenceRows:null);if(provided){return provided.map((item,index)=>({point:Number(item?.point||0),title:item?.label||item?.title||item?.key||'一致項目',value:item?.value||'',source:item?.source||item?.sourceLabel||'スコア根拠',condition:item?.condition||'常に',matchedText:item?.matchedText||'',rawText:item?.rawText||item?.matchedText||'',evidenceType:item?.evidenceType||item?.sourceKind||'detail',reason:item?.reason||'',index}));}const out=[];const seen=new Set();const fallbackPoint=Math.max(1,Math.floor(Number(row?.score||0)/Math.max(1,(row?.matchedEffects?.length||0)+(row?.matchedParameters?.length||0))));const push=(src,type)=>{if(!src)return;const title=src.label||src.key||'一致項目';const value=src.value||'';const source=src.sourceLabel||(type==='parameter'?'変化率集計':'');const condition=formationScoreEvidenceCondition(src);const key=[norm(title),norm(source),norm(value),norm(src.timing||''),norm(condition)].join('|');if(seen.has(key))return;seen.add(key);out.push({point:fallbackPoint,title,value,source:source||'スコア根拠',condition,matchedText:src.matchedText||'',rawText:src.rawText||src.matchedText||'',evidenceType:type,reason:`fallback ${type}: スコア内訳データがないため根拠として表示`});};(Array.isArray(row?.matchedEffects)?row.matchedEffects:[]).forEach(v=>push(v,'effect'));(Array.isArray(row?.matchedParameters)?row.matchedParameters:[]).forEach(v=>push(v,'parameter'));return out;}
const FORMATION_SCORE_FALLBACK_LABELS=['評価1','評価2','評価3','評価4','評価5'];
function normalizeFormationScoreDisplayRows(rows){const src=Array.isArray(rows)?rows:[];const out=src.slice(0,5).map((row,index)=>({label:row?.label||FORMATION_SCORE_FALLBACK_LABELS[index]||`評価${index+1}`,score:Number(row?.score||0),unit:row?.unit||'',scoreDetails:Array.isArray(row?.scoreDetails)?row.scoreDetails:(Array.isArray(row?.evidenceRows)?row.evidenceRows:[]),evidenceRows:Array.isArray(row?.evidenceRows)?row.evidenceRows:(Array.isArray(row?.scoreDetails)?row.scoreDetails:[]),matchedEffects:Array.isArray(row?.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row?.matchedParameters)?row.matchedParameters:[]}));while(out.length<5){const idx=out.length;out.push({label:FORMATION_SCORE_FALLBACK_LABELS[idx]||`評価${idx+1}`,score:0,unit:'',scoreDetails:[],evidenceRows:[],matchedEffects:[],matchedParameters:[]});}return out;}
function renderFormationScoreMetricChipsHtml(rows,selectedIndex){return `<div class="formation-score-metric-chips" role="tablist" aria-label="評価スコア項目">${rows.map((row,index)=>{const evidenceCount=normalizeFormationScoreEvidenceRows(row).length;return `<button type="button" class="formation-score-metric-chip ${index===selectedIndex?'is-active':''} ${Number(row.score||0)>0?'':'is-zero'}" data-formation-score-detail-index="${esc(index)}" data-formation-score-detail-label="${esc(row.label||'')}" data-formation-score-detail-score="${esc(row.score||0)}" data-formation-score-detail-evidence-count="${esc(evidenceCount)}" aria-pressed="${index===selectedIndex?'true':'false'}"><span>${esc(row.label)}</span><strong>${esc(row.score)}</strong><small>根拠${esc(evidenceCount)}件</small></button>`;}).join('')}</div>`;}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const score=Number(row.score||0);const detailTotal=sumFormationScoreDetails(evidenceRows);if(evidenceRows.length&&detailTotal!==score)debugLog('formationScore:score-detail-mismatch',{label:row.label||'',score,detailTotal,detailCount:evidenceRows.length});debugLog('formationScore:evidence-summary',{label:row.label||'',score,detailTotal,evidenceCount:evidenceRows.length});const expanded=!!state.formationScoreDetailExpanded;const limit=expanded?30:8;const visible=evidenceRows.slice(0,limit);const hidden=Math.max(0,evidenceRows.length-visible.length);const body=visible.length?`<div class="formation-score-evidence-list">${visible.map(item=>`<div class="formation-score-evidence-item"><span class="formation-score-evidence-title" title="${esc(item.title)}">${esc(item.title)}</span><span class="formation-score-evidence-value">${esc(item.value)}</span><span class="formation-score-evidence-source" title="${esc(item.source)}">発生元：${esc(item.source)}</span><span class="formation-score-evidence-condition">条件：${esc(item.condition||'常に')}</span><span class="formation-score-evidence-source" title="${esc(item.reason||'')}">理由：${esc(item.reason||'スコア条件に一致')}</span><span class="formation-score-evidence-condition" title="${esc(item.rawText||item.matchedText||'')}">原文：${esc(item.rawText||item.matchedText||'')}</span></div>`).join('')}</div>`:`<div class="formation-score-empty"><strong>一致根拠なし</strong><span>${esc(row.label)}は評価${esc(score)}ですが、一致根拠データが生成されていません。</span></div>`;const more=evidenceRows.length>8?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">${expanded?'表示を絞る':`さらに表示（ほか${esc(Math.max(0,evidenceRows.length-8))}件）`}</button>`:'';const overflow=hidden>0&&expanded?`<div class="formation-score-more-note">ほか${esc(hidden)}件</div>`:'';return `<div class="formation-score-detail-panel"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>評価${esc(score)} / 根拠${esc(evidenceRows.length)}件</span></div>${body}${more}${overflow}</div>`;}
...
function toggleFormationScoreDetailMore(){state.formationScoreDetailExpanded=!state.formationScoreDetailExpanded;renderFormationScreen();}
function formationScoreDetailPayload(btn){return {previousIndex:Number.isInteger(state.formationScoreDetailIndex)?state.formationScoreDetailIndex:0,nextIndex:Number(btn?.dataset?.formationScoreDetailIndex)||0,rowLabel:btn?.dataset?.formationScoreDetailLabel||'',rowScore:Number(btn?.dataset?.formationScoreDetailScore)||0,evidenceCount:Number(btn?.dataset?.formationScoreDetailEvidenceCount)||0,rendered:true};}
function updateFormationScoreDetailCard(btn,payload){const card=btn?.closest?btn.closest('.formation-score-card'):null;const rows=Array.isArray(state.formationScoreDetailRows)?state.formationScoreDetailRows:[];const row=rows[payload.nextIndex]||{label:payload.rowLabel||'評価項目',score:payload.rowScore||0,matchedEffects:[],matchedParameters:[]};state.formationScoreDetailIndex=payload.nextIndex;state.formationScoreDetailExpanded=false;if(card){card.querySelectorAll('[data-formation-score-detail-index]').forEach(chip=>{const active=Number(chip.dataset.formationScoreDetailIndex)===payload.nextIndex;chip.classList.toggle('is-active',active);chip.setAttribute('aria-pressed',active?'true':'false');});const currentPanel=card.querySelector('.formation-score-detail-panel');const nextPanelHtml=renderFormationScoreEvidencePanelHtml(row);if(currentPanel&&nextPanelHtml){currentPanel.outerHTML=nextPanelHtml;}}return row;}
function handleFormationScoreDetailClick(btn,event){if(event){event.preventDefault();event.stopPropagation();}const payload=formationScoreDetailPayload(btn);const row=updateFormationScoreDetailCard(btn,payload);payload.rowLabel=row?.label||payload.rowLabel;payload.rowScore=Number(row?.score||payload.rowScore)||0;payload.evidenceCount=normalizeFormationScoreEvidenceRows(row).length;debugLog('formationScore:detail-click',payload);debugLog('formationScoreDetail:click',payload);}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const matchedCount=displayTotalScore;
  const evidenceCount=displayTotalScore;
  ...
  const renderPayload={formationId:f?.id||'',formationName:f?.name||'',typeName:scores.breakdown.evaluationTypeName||'',totalScore:displayTotalScore,evaluationScore:displayTotalScore,rowCount:rows.length,matchedCount,evidenceCount,selectedIndex,emptyReason:emptyReason||'',rendered:true,uiRendered:true,policy:'always-visible score card in formation-detail-panel'};
  ...
  return `<section class="formation-selected-card formation-score-card no-detail-linkify" aria-label="自動計算スコア" data-formation-score-card="1"><div class="formation-score-summary"><div class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(displayTotalScore)}</strong><small>${esc(metaText)}</small></div></div><div class="formation-score-summary-body">${metricChips}${detailPanel}${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></div></section>`;
}
function renderFormationGroupNameDialogHtml(){if(!state.formationGroupNameDialogOpen)return '';const groups=sanitizeFormationGroups(state.formationGroups||[]),group=getCurrentFormationGroup();return `<div class="formation-mobile-dialog-overlay formation-group-dialog-overlay" data-formation-dialog-backdrop="group-name"><div class="formation-mobile-dialog-card" role="dialog" aria-modal="true" aria-label="グループ変更"><div class="formation-mobile-dialog-head"><div class="formation-mobile-dialog-title">グループ変更</div><button type="button" class="formation-mobile-dialog-close" data-formation-group-name-close="1">閉じる</button></div><label class="formation-dialog-field"><span class="note">グループリスト</span><select id="formationGroupDialogSelect" class="formation-select">${groups.map(g=>`<option value="${esc(g.id)}" ${g.id===group.id?'selected':''}>${esc(g.name)}</option>`).join('')}</select></label><label class="formation-dialog-field"><span class="note">グループ名</span><input id="formationGroupNameDialogInput" type="text" value="${esc(group.name)}" maxlength="32"></label><div class="formation-dialog-actions"><button type="button" id="formationGroupCreateBtn" ${groups.length>=FORMATION_MAX_GROUPS?'disabled':''}>新規作成</button><button type="button" id="formationGroupNameSaveBtn" class="btn-select-all">名前変更</button><button type="button" id="formationGroupDeleteBtn" class="btn-clear-all" ${groups.length<=1?'disabled':''}>削除</button><button type="button" data-formation-group-name-close="1">閉じる</button></div></div></div>`;}
...
function renderFormationGroupControlsHtml(){const groups=sanitizeFormationGroups(state.formationGroups||[]),current=getCurrentFormationGroup();return `<div class="formation-group-controls"><div class="formation-group-head"><div><span class="formation-group-title">グループ</span><span class="formation-group-count">${esc(groups.length)}/${esc(FORMATION_MAX_GROUPS)}</span></div><button type="button" id="formationGroupRenameBtn" data-formation-group-manage="1" class="formation-group-manage-btn" aria-label="グループ変更ダイアログを開く">変更</button></div><label class="formation-group-select-label"><span class="note">グループリスト</span><select id="formationGroupSelect" data-formation-group-select="1" class="formation-select">${groups.map(g=>`<option value="${esc(g.id)}" ${g.id===current.id?'selected':''}>${esc(g.name)}</option>`).join('')}</select></label><div class="formation-group-help note">選択中グループごとに最大${esc(FORMATION_MAX_PER_GROUP)}部隊を管理します。変更から作成・名前変更・削除を行います。</div></div>`;}
function renderFormationListHtml(){const visible=getVisibleFormations();return visible.map((f,i)=>{const siege=formationExtensionDisplayName('siegeWeapon',f.siegeWeapon||{});const arm=formationExtensionDisplayName('ethnicArmament',f.ethnicArmament||{});const scores=calculateFormationAutoScores(f,{});return `<button type="button" class="formation-list-item ${f.id===state.currentFormationId?'is-active':''}" data-formation-select="${esc(f.id)}"><span class="formation-list-no">${i+1}</span><span><span class="formation-list-name">${esc(f.name)}</span><span class="formation-list-meta">型:${esc(formationEvaluationTypeDisplayName(f))} / 合計:${esc(scores.totalScore)}<br>更新: ${esc(formatFormationDate(f.updatedAt))}<br>兵器:${esc(siege)} / 武装:${esc(arm)}</span></span><span>${f.id===state.currentFormationId?'編集中':'›'}</span></button>`;}).join('')||'<div class="detail-empty">このグループに部隊がありません</div>';}
