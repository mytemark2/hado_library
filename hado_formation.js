...
function formationScoreRowToDebug(row){const text=String(row?.matchedText||'');return {featureId:row?.featureId||'',label:row?.label||row?.statusEffectName||'',sourceKind:row?.sourceKind||'',sourceLabel:row?.sourceLabel||'',key:row?.key||'',group:row?.group||'',timing:row?.timing||'',value:row?.value||'',condition:row?.condition||'',matchedText:text.slice(0,500),rawText:String(row?.rawText||text).slice(0,1000)};}
...
function summarizeFormationRuleScore(rule,scoreResult){const rows=(scoreResult?.breakdown||[]).slice(0,5).map(row=>{const matched=Array.isArray(row?.rows)?row.rows:[];const matchedEffects=matched.filter(v=>v?.sourceKind==='effect').map(formationScoreRowToDebug).slice(0,20);const matchedParameters=matched.filter(v=>v?.sourceKind==='parameter').map(formationScoreRowToDebug).slice(0,20);const score=matchedEffects.length+matchedParameters.length;return {label:row?.label||row?.metricKey||'',score,matchedEffects,matchedParameters};});return {typeId:rule?.typeId||'',typeName:rule?.typeName||rule?.typeId||'',totalScore:rows.reduce((sum,row)=>sum+Number(row.score||0),0),rows};}
...
function calculateFormationAutoScores(f,data={}){let diag;try{diag=calculateFormationTypeScore(f,data);}catch(err){debugLog('formationScore:empty',{formationId:f?.id||'',formationName:f?.name||'',reason:'score calculation error',message:err?.message||String(err)});return createFormationFallbackAutoScores(f,data,`score calculation error: ${err?.message||String(err)}`);}const selected=diag.candidateScores?.[0]||null;const scoreRows=(selected?.rows||[]).slice(0,5).map(row=>({label:row.label,score:row.score,unit:'件',matchedEffects:Array.isArray(row.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row.matchedParameters)?row.matchedParameters:[]}));const totalScore=scoreRows.reduce((sum,row)=>sum+Number(row.score||0),0);return {totalScore,evaluationScore:totalScore,breakdown:{scoreRows,filledGenerals:countFormationFilledGenerals(f),skillCount:Array.isArray(data?.skills)?data.skills.length:0,parameterCount:diag.effectSourceCount||diag.parameterRowCount||0,evaluationTypeName:selected?.typeName||formationEvaluationTypeDisplayName(f),scorePolicy:diag.selectedTypeId?'formation-effect-score-selected-type':'formation-effect-score-candidates',memberCount:0,candidateScores:diag.candidateScores||[],emptyReason:diag.emptyReason||''}};}
...
const FORMATION_SCORE_EVIDENCE_ALIASES={
  '自部隊不利対策':['弱化無効','弱化効果無効','弱化解除','弱化効果解除','状態変化無効','弱化回避','弱化効果回避','弱化反射','弱化効果反射','会心無効','撃心無効','不利変化無効']
};
function formationScoreEvidenceTitle(src,row){const fallback=src?.label||src?.key||'一致項目';const aliases=FORMATION_SCORE_EVIDENCE_ALIASES[row?.label||'']||[];const text=`${src?.matchedText||''} ${src?.rawText||''}`;const hit=aliases.find(alias=>text.includes(alias));return hit||fallback;}
function formationScoreEvidenceKind(type){return type==='effect'?{kind:'status',kindLabel:'状態変化'}:{kind:'type',kindLabel:'型要素'};}
function sumFormationScoreDetails(details){return (Array.isArray(details)?details:[]).reduce((sum,item)=>sum+(Number(item?.point??item?.score??1)||0),0);}
function formationScoreEvidenceSourceTag(src){if(src?.sourceKind==='parameter')return String(src?.group||'変化率').trim().slice(0,18);const raw=String(src?.sourceLabel||src?.source||src?.group||src?.key||'タグ根拠').trim();const parts=raw.split(':').map(v=>v.trim()).filter(Boolean);return (parts[parts.length-1]||raw).replace(/[［\[].*?[］\]]/g,'').slice(0,18);}
function normalizeFormationScoreEvidenceRows(row){const out=[];const push=(src,type)=>{if(!src)return;const title=formationScoreEvidenceTitle(src,row);if(!title)return;out.push({point:1,title,sourceTag:formationScoreEvidenceSourceTag(src),evidenceType:type,...formationScoreEvidenceKind(type)});};(Array.isArray(row?.matchedEffects)?row.matchedEffects:[]).forEach(v=>push(v,'effect'));(Array.isArray(row?.matchedParameters)?row.matchedParameters:[]).forEach(v=>push(v,'parameter'));const counts=out.reduce((acc,item)=>{acc[item.title]=(acc[item.title]||0)+1;return acc;},{});return out.map(item=>({...item,displayTitle:item.sourceTag?`${item.title}（${item.sourceTag}）`:item.title}));}
const FORMATION_SCORE_FALLBACK_LABELS=['評価1','評価2','評価3','評価4','評価5'];
function normalizeFormationScoreDisplayRows(rows){const src=Array.isArray(rows)?rows:[];const out=src.slice(0,5).map((row,index)=>({label:row?.label||FORMATION_SCORE_FALLBACK_LABELS[index]||`評価${index+1}`,score:Number(row?.score||0),unit:row?.unit||'件',matchedEffects:Array.isArray(row?.matchedEffects)?row.matchedEffects:[],matchedParameters:Array.isArray(row?.matchedParameters)?row.matchedParameters:[]}));while(out.length<5){const idx=out.length;out.push({label:FORMATION_SCORE_FALLBACK_LABELS[idx]||`評価${idx+1}`,score:0,unit:'件',matchedEffects:[],matchedParameters:[]});}return out;}
function renderFormationScoreMetricChipsHtml(rows,selectedIndex){return `<div class="formation-score-metric-chips" role="tablist" aria-label="評価スコア項目">${rows.map((row,index)=>{const evidenceCount=normalizeFormationScoreEvidenceRows(row).length;return `<button type="button" class="formation-score-metric-chip ${index===selectedIndex?'is-active':''} ${evidenceCount>0?'':'is-zero'}" data-formation-score-detail-index="${esc(index)}" data-formation-score-detail-label="${esc(row.label||'')}" data-formation-score-detail-score="${esc(evidenceCount)}" data-formation-score-detail-evidence-count="${esc(evidenceCount)}" aria-pressed="${index===selectedIndex?'true':'false'}"><span>${esc(row.label)}</span><strong>${esc(evidenceCount)}</strong></button>`;}).join('')}</div>`;}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const expanded=!!state.formationScoreDetailExpanded;const limit=expanded?30:12;const visible=evidenceRows.slice(0,limit);const hidden=Math.max(0,evidenceRows.length-visible.length);const body=visible.length?`<div class="formation-score-evidence-tags">${visible.map(item=>`<span class="formation-score-evidence-tag is-${esc(item.kind||'type')}"><b>${esc(item.kindLabel||'型要素')}</b>${esc(item.displayTitle||item.title)}</span>`).join('')}</div>`:'<div class="formation-score-empty">該当タグなし</div>';const more=evidenceRows.length>12?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">${expanded?'表示を絞る':`さらに表示（ほか${esc(Math.max(0,evidenceRows.length-12))}件）`}</button>`:'';const overflow=hidden>0&&expanded?`<div class="formation-score-more-note">ほか${esc(hidden)}件</div>`:'';return `<div class="formation-score-detail-panel"><div class="formation-score-detail-head"><strong>${esc(row.label)}のタグ</strong><span>${esc(evidenceRows.length)}タグ</span></div>${body}${more}${overflow}</div>`;}
...
function toggleFormationScoreDetailMore(event){if(event){event.preventDefault();event.stopPropagation();}state.formationScoreDetailExpanded=!state.formationScoreDetailExpanded;debugLog('formationScore:detail-more',{expanded:!!state.formationScoreDetailExpanded});renderFormationScreen();}
...
function handleFormationScoreDetailClick(btn,event){if(event){event.preventDefault();event.stopPropagation();}const payload=formationScoreDetailPayload(btn);debugLog('formationScore:detail-click',payload);debugLog('formationScoreDetail:click',payload);setFormationScoreDetailIndex(payload.nextIndex);}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const displayTotalScore=rows.reduce((sum,row)=>sum+normalizeFormationScoreEvidenceRows(row).length,0);f.totalScore=displayTotalScore;f.evaluationScore=displayTotalScore;
