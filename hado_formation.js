...
function normalizeFormationScoreEvidenceRows(row){const out=[];const push=(src,type)=>{if(!src)return;const title=formationScoreEvidenceTitle(src,row);if(!title)return;out.push({point:1,title,evidenceType:type,...formationScoreEvidenceKind(type)});};(Array.isArray(row?.matchedEffects)?row.matchedEffects:[]).forEach(v=>push(v,'effect'));(Array.isArray(row?.matchedParameters)?row.matchedParameters:[]).forEach(v=>push(v,'parameter'));return out;}
