...
function renderFormationScoreEvidenceItemsHtml(evidenceRows,expanded=false){return (evidenceRows||[]).map(item=>`<span class="formation-score-evidence-tag formation-quick-summary-chip is-${esc(item.kind||'type')}"><b>${esc(item.kindLabel||'型要素')}</b><span class="formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span></span>`).join('');}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const visible=evidenceRows.slice(0,12);const body=visible.length?`<div class="formation-score-evidence-tags is-collapsed" aria-label="${esc(row.label||'評価スコア')}の根拠">${renderFormationScoreEvidenceItemsHtml(visible,false)}</div>`:'<div class="formation-score-empty">該当タグなし</div>';const more=evidenceRows.length>12?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">さらに表示（全${esc(evidenceRows.length)}件）</button>`:'';return `<div class="formation-score-detail-panel is-collapsed"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>評価${esc(evidenceRows.length)} / 根拠${esc(evidenceRows.length)}件</span></div>${body}${more}</div>`;}
function renderFormationScoreEvidenceDialogHtml(row){if(!state.formationScoreEvidenceDialogOpen||!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);return `<div class="formation-mobile-dialog-overlay formation-score-evidence-dialog-overlay" data-formation-score-dialog-backdrop="1"><div class="formation-mobile-dialog-card formation-score-evidence-dialog-card" role="dialog" aria-modal="true" aria-label="${esc(row.label||'評価スコア')}の内訳"><div class="formation-mobile-dialog-head"><div class="formation-mobile-dialog-title">${esc(row.label||'評価スコア')}の内訳</div><button type="button" class="formation-mobile-dialog-close" data-formation-score-dialog-close="1">閉じる</button></div><div class="formation-score-detail-head"><strong>評価${esc(evidenceRows.length)} / 根拠${esc(evidenceRows.length)}件</strong><span>全件表示</span></div>${evidenceRows.length?`<div class="formation-score-evidence-tags is-expanded formation-score-evidence-dialog-list" aria-label="${esc(row.label||'評価スコア')}の根拠全件">${renderFormationScoreEvidenceItemsHtml(evidenceRows,true)}</div>`:'<div class="formation-score-empty">該当タグなし</div>'}<div class="formation-note">結果サマリーと同じチップ形式で、選択中評価スコアの根拠を全件表示しています。</div></div></div>`;}
...
function toggleFormationScoreDetailMore(event){if(event){event.preventDefault();event.stopPropagation();}state.formationScoreEvidenceDialogOpen=true;debugLog('formationScore:detail-more',{dialogOpen:true,index:state.formationScoreDetailIndex||0});renderFormationScreen();}
...
function handleFormationScoreDetailClick(btn,event){if(event){event.preventDefault();event.stopPropagation();}const payload=formationScoreDetailPayload(btn);debugLog('formationScore:detail-click',payload);debugLog('formationScoreDetail:click',payload);state.formationScoreDetailIndex=payload.nextIndex;state.formationScoreDetailExpanded=false;state.formationScoreEvidenceDialogOpen=false;const card=btn&&typeof btn.closest==='function'?btn.closest('.formation-score-card'):null;if(card){card.querySelectorAll('[data-formation-score-detail-index]').forEach(chip=>{const active=Number(chip.dataset?.formationScoreDetailIndex)===payload.nextIndex;chip.classList?.toggle('is-active',active);if(typeof chip.setAttribute==='function')chip.setAttribute('aria-pressed',active?'true':'false');});const rows=state.formationScoreDetailRows||[];const panel=card.querySelector('.formation-score-detail-panel');if(panel&&rows[payload.nextIndex])panel.outerHTML=renderFormationScoreEvidencePanelHtml(rows[payload.nextIndex]).replace('のタグ','の内訳').replace('該当タグなし','一致根拠なし');return;}renderFormationScreen();}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  return `<section class="formation-selected-card formation-score-card no-detail-linkify" aria-label="自動計算スコア" data-formation-score-card="1"><div class="formation-score-summary"><div class="formation-score-summary-head">${typeSelectHtml}<div class="formation-score-total"><span>トータルスコア</span><strong>${esc(visibleTotalScore)}</strong><small>${esc(metaText)}</small></div></div><div class="formation-score-summary-body">${metricChips}${detailPanel}${detailDialog}${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></div></section>`;
}
...
function formationCandidateTraySnapshot(){const f=ensureCurrentFormation();return {formationId:f.id,formationName:f.name,evaluationTypeId:f.evaluationTypeId||'',evaluationTypeName:f.evaluationTypeName||'',items:sanitizeFormationCandidateTray(f.candidateTray||[])}}
...
function placeFormationCandidateTrayItem(id){const f=ensureCurrentFormation();const row=sanitizeFormationCandidateTray(f.candidateTray||[]).find(x=>x.id===norm(id));if(!row)return;const category=candidateTrayCategoryForRole(row.roleId);if(!['generals','equipments'].includes(category)){showFormationToast('この候補は候補保存のみです');dispatchFormationCandidateTraySnapshot('candidate-tray-place-unsupported');return;}const item=findItemByDisplayName(category,row.name)||findItemByDisplayName(category,row.displayName);if(!item){showFormationToast('配置対象が見つかりません');dispatchFormationCandidateTraySnapshot('candidate-tray-place-missing');return;}setMainTab('formation');renderFormationAddPopover(item);dispatchFormationCandidateTraySnapshot('candidate-tray-place');}
