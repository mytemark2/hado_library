...
function formationScoreRowToDebug(row){return {featureId:row?.featureId||'',label:row?.label||row?.statusEffectName||'',sourceKind:row?.sourceKind||'',sourceLabel:row?.sourceLabel||'',key:row?.key||'',group:row?.group||'',timing:row?.timing||'',value:row?.value||'',condition:row?.condition||'',matchedText:String(row?.matchedText||'').slice(0,160)};}
...
function formationScoreEvidenceTitle(src,row){const fallback=src?.label||src?.key||'一致項目';const aliases=FORMATION_SCORE_EVIDENCE_ALIASES[row?.label||'']||[];const text=String(src?.matchedText||'');const hit=aliases.find(alias=>text.includes(alias));return hit||fallback;}
