...
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const expanded=!!state.formationScoreDetailExpanded;const limit=expanded?30:12;const visible=evidenceRows.slice(0,limit);const hidden=Math.max(0,evidenceRows.length-visible.length);const body=visible.length?`<div class="formation-score-evidence-tags ${expanded?'is-expanded':'is-collapsed'}" aria-label="${esc(row.label||'評価スコア')}の根拠">${visible.map(item=>`<span class="formation-score-evidence-tag formation-quick-summary-chip is-${esc(item.kind||'type')}"><b>${esc(item.kindLabel||'型要素')}</b><span class="formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span></span>`).join('')}</div>`:'<div class="formation-score-empty">該当タグなし</div>';const more=evidenceRows.length>12?`<button type="button" class="formation-score-show-more" data-formation-score-show-more="1">${expanded?'表示を絞る':`さらに表示（ほか${esc(Math.max(0,evidenceRows.length-12))}件）`}</button>`:'';const overflow=hidden>0&&expanded?`<div class="formation-score-more-note">ほか${esc(hidden)}件</div>`:'';return `<div class="formation-score-detail-panel ${expanded?'is-expanded':'is-collapsed'}"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>評価${esc(evidenceRows.length)} / 根拠${esc(evidenceRows.length)}件</span></div>${body}${more}${overflow}</div>`;}
