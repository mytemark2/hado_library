...
function handleFormationScoreDetailClick(btn,event){if(event){event.preventDefault();event.stopPropagation();}const previousIndex=Number.isInteger(state.formationScoreDetailIndex)?state.formationScoreDetailIndex:0;const nextIndex=Number(btn?.dataset?.formationScoreDetailIndex)||0;const payload={previousIndex,nextIndex,rowLabel:btn?.dataset?.formationScoreDetailLabel||'',rowScore:Number(btn?.dataset?.formationScoreDetailScore)||0,evidenceCount:Number(btn?.dataset?.formationScoreDetailEvidenceCount)||0,rendered:true};debugLog('formationScore:detail-click',payload);debugLog('formationScoreDetail:click',payload);setFormationScoreDetailIndex(nextIndex);}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const renderPayload={formationId:f?.id||'',formationName:f?.name||'',typeName:scores.breakdown.evaluationTypeName||'',totalScore:scores.totalScore,evaluationScore:scores.evaluationScore,rowCount:rows.length,matchedCount,evidenceCount,selectedIndex,emptyReason:emptyReason||'',rendered:true,uiRendered:true,policy:'always-visible score card in formation-detail-panel'};
  ...
  return `<section class="formation-selected-card formation-score-card no-detail-linkify" aria-label="自動計算スコア" data-formation-score-card="1"><div class="formation-score-summary"><div class="formation-score-summary-head"><div class="formation-score-total"><span>トータルスコア</span><strong>${esc(scores.totalScore)}</strong><small>${esc(metaText)}</small></div></div><div class="formation-score-summary-body">${metricChips}${detailPanel}${candidateRows}<div class="formation-score-breakdown formation-score-meta">${meta.map(([label,value])=>`<span><span class="formation-score-label">${esc(label)}</span>${esc(value)}</span>`).join('')}</div>${emptyNote}</div></div></section>`;
}
...
function renderFormationGroupControlsHtml(){const groups=sanitizeFormationGroups(state.formationGroups||[]),current=getCurrentFormationGroup();return `<div class="formation-group-controls"><div class="formation-group-head"><span class="formation-group-title">グループ</span><button type="button" id="formationGroupRenameBtn" class="formation-group-manage-btn">変更</button></div><label class="formation-group-select-label"><span class="note">グループリスト</span><select id="formationGroupSelect" class="formation-select">${groups.map(g=>`<option value="${esc(g.id)}" ${g.id===current.id?'selected':''}>${esc(g.name)}</option>`).join('')}</select></label></div>`;}
...
function setupFormationUpdate08Events(){
  ...
  const groupRename=document.getElementById('formationGroupRenameBtn');
  if(groupRename)groupRename.addEventListener('click',openFormationGroupNameDialog);
