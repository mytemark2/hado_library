...
function summarizeFormationRuleScore(rule,scoreResult){const rows=(scoreResult?.breakdown||[]).slice(0,5).map(row=>{const matched=Array.isArray(row?.rows)?row.rows:[];return {label:row?.label||row?.metricKey||'',score:Number(row?.itemCount||row?.conditionalMaxValue||0),matchedEffects:matched.filter(v=>v?.sourceKind==='effect').map(formationScoreRowToDebug).slice(0,20),matchedParameters:matched.filter(v=>v?.sourceKind==='parameter').map(formationScoreRowToDebug).slice(0,20)};});return {typeId:rule?.typeId||'',typeName:rule?.typeName||rule?.typeId||'',totalScore:rows.reduce((sum,row)=>sum+Number(row.score||0),0),rows};}
...
function formationScoreEvidenceTitle(src,row){const fallback=src?.label||src?.key||'一致項目';const aliases=FORMATION_SCORE_EVIDENCE_ALIASES[row?.label||'']||[];const text=`${src?.matchedText||''} ${src?.rawText||''}`;const hit=aliases.find(alias=>text.includes(alias));if(hit)return hit;if((row?.label||'')==='自部隊不利対策'&&/与ダメージ|被ダメージ/.test(fallback))return '不利対策根拠';return fallback;}
function normalizeFormationScoreEvidenceRows(row){const out=[];const seen=new Set();const push=(src,type)=>{if(!src)return;const title=formationScoreEvidenceTitle(src,row);const rawLabel=src.label||src.key||'';const value=title===rawLabel?(src.value||''):'';const source=src.sourceLabel||(type==='parameter'?'変化率集計':'');const condition=formationScoreEvidenceCondition(src);const key=[norm(title),norm(source),norm(value),norm(src.timing||''),norm(condition)].join('|');if(seen.has(key))return;seen.add(key);out.push({point:1,title,value,source:source||'スコア根拠',condition,evidenceType:type});};(Array.isArray(row?.matchedEffects)?row.matchedEffects:[]).forEach(v=>push(v,'effect'));(Array.isArray(row?.matchedParameters)?row.matchedParameters:[]).forEach(v=>push(v,'parameter'));return out;}
...
function renderFormationScoreSummaryHtml(f,data){
  ...
  const metaText=`${scores.breakdown.evaluationTypeName} / ${rows.length}項目 / ${matchedCount}件一致`;
