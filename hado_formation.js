...
function formationScoreEvidenceSimpleTitle(item){return String(item?.displayBucket||item?.title||item?.label||item?.key||'一致項目').trim()||'一致項目';}
...
function renderFormationScoreEvidenceItemsHtml(evidenceRows,expanded=false){const rows=expanded?(evidenceRows||[]):uniqueFormationScoreEvidenceSummaryRows(evidenceRows);return rows.map(item=>expanded?`<span class="formation-score-evidence-tag formation-quick-summary-chip is-${esc(item.kind||'type')}"><b>${esc(item.kindLabel||'型要素')}</b><span class="formation-score-evidence-label">${esc(item.displayTitle||item.title)}</span></span>`:`<span class="formation-score-evidence-tag formation-quick-summary-chip is-summary is-${esc(item.kind||'type')}"><span class="formation-score-evidence-label">${esc(item.simpleTitle||formationScoreEvidenceSimpleTitle(item))}</span></span>`).join('');}
function renderFormationScoreEvidencePanelHtml(row){if(!row)return '';const evidenceRows=normalizeFormationScoreEvidenceRows(row);const summaryRows=uniqueFormationScoreEvidenceSummaryRows(evidenceRows);const body=summaryRows.length?`<div class="formation-score-evidence-tags is-collapsed" aria-label="${esc(row.label||'評価スコア')}の簡略根拠">${renderFormationScoreEvidenceItemsHtml(summaryRows,false)}</div>`:'<div class="formation-score-empty">該当タグなし</div>';return `<div class="formation-score-detail-panel is-collapsed is-clickable" data-formation-score-detail-open="1" role="button" tabindex="0" aria-label="${esc(row.label||'評価スコア')}の詳細を表示"><div class="formation-score-detail-head"><strong>${esc(row.label)}の内訳</strong><span>クリックで詳細表示</span></div>${body}</div>`;}
