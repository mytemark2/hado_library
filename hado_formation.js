...
function formationScoreEvidenceTitle(src,row){const fallback=src?.label||src?.key||'一致項目';const aliases=FORMATION_SCORE_EVIDENCE_ALIASES[row?.label||'']||[];const text=`${src?.matchedText||''} ${src?.rawText||''}`;const hit=aliases.find(alias=>text.includes(alias));if(hit)return hit;return aliases.length?'':fallback;}
function normalizeFormationScoreEvidenceRows(row){const out=[];const seen=new Set();const push=(src,type)=>{if(!src)return;const title=formationScoreEvidenceTitle(src,row);if(!title)return;const key=norm(title);if(seen.has(key))return;seen.add(key);out.push({point:1,title,evidenceType:type});};(Array.isArray(row?.matchedEffects)?row.matchedEffects:[]).forEach(v=>push(v,'effect'));(Array.isArray(row?.matchedParameters)?row.matchedParameters:[]).forEach(v=>push(v,'parameter'));return out;}
