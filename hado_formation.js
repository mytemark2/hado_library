...
function renderFormationNextStepHelpHtml(){return `<details class="formation-next-step-help"><summary>次の操作</summary><div class="formation-next-step-body"><ol><li>グループ切替で攻城・防衛・イベントなどの表示対象を選びます。</li><li>グループを整理する場合は「変更」から追加・名前変更・削除を行います。</li><li>部隊を選び、編成盤面の枠を押して候補トレイや検索結果から配置します。</li><li>トータルスコアと評価タグを確認し、問題なければ「保存」で現在の部隊へ反映します。</li></ol></div></details>`;}
function renderFormationGroupControlsHtml(){const groups=sanitizeFormationGroups(state.formationGroups||[]),current=getCurrentFormationGroup();return `<div class="formation-group-controls"><div class="formation-group-head"><span class="formation-group-title">グループ</span><span class="formation-group-current-name" title="${esc(current.name)}">${esc(current.name)}</span><button type="button" id="formationGroupRenameBtn" class="formation-group-manage-btn" data-formation-group-manage="1" aria-label="部隊グループを変更">変更</button></div><label class="formation-group-select-label"><span class="note">切替</span><select id="formationGroupSelect" class="formation-select" data-formation-group-select="1" aria-label="部隊グループを切り替え">${groups.map(g=>`<option value="${esc(g.id)}" ${g.id===current.id?'selected':''}>${esc(g.name)}</option>`).join('')}</select></label><span class="formation-group-count">${esc(groups.length)}グループ</span>${renderFormationNextStepHelpHtml()}</div>`;}
