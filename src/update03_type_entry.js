...
(() => {
  ...
  function scorePurpose(general, p) {
    const rows = (p.recommendedTypeIds || []).map((typeId) => ({ rule: state.data.scoreRules.find((r) => r.typeId === typeId), typeId }))
      .filter((v) => v.rule).map((v) => ({ ...v, result: scoreType(general, v.rule) }))
  ...
  function renderPurposeStep() {
    ...
    return `<div class="hte-card"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div class="hte-title">${state.mainGeneral ? '主将に合う目的を選択' : '目的を選択'}</div>${state.mainGeneral ? `<label class="hte-note"><input type="checkbox" id="hadoShowAllPurposes" ${state.showAllPurposes ? 'checked' : ''}> 全目的を表示</label>` : ''}</div><div class="hte-list">${visible.map(({p,r}) => `<button class="hte-item ${state.purposeId === p.purposeId ? 'active' : ''}" data-purpose-id="${esc(p.purposeId)}"><div class="hte-title">${esc(p.purposeName)}</div><div class="hte-score">主将参考適合度: ${r.score == null ? '-' : `${r.score}/10`}</div>${r.best ? `<div class="hte-match">最上位型: ${esc(r.best.rule.typeName)} / ${r.best.result.matched.length}/5項目一致</div>` : ''}</button>`).join('') || '<div class="hte-note">一致する目的がありません。「全目的を表示」で全件確認できます。</div>'}</div></div>`;
  }
  function renderTypeStep() {
    const rules = typeRows(state.mode === 'type' ? null : purpose()?.recommendedTypeIds);
    return `<div class="hte-card"><div class="hte-title">型を選択</div><div class="hte-list">${rules.map((rule) => { const r = scoreType(state.mainGeneral, rule); return `<button class="hte-item ${state.typeId === rule.typeId ? 'active' : ''}" data-type-id="${esc(rule.typeId)}"><div class="hte-title">${esc(rule.typeName)}</div><div class="hte-score">主将参考適合度: ${r.score == null ? '-' : `${r.score}/10`}</div><div class="hte-match">一致: ${r.matched.length}/5項目${r.matched.length ? ` / ${r.matched.map((m) => esc(m.label)).join('、')}` : ''}</div></button>`; }).join('')}</div><div class="hte-note">固定5項目を各2点で採点します。候補数や広い特徴語では加点しません。</div></div>`;
  }
  function renderConfirmStep() {
    return `<div class="hte-card"><div class="hte-title">選択内容を確認</div><div class="hte-confirm"><div class="hte-confirm-row"><div class="hte-confirm-label">選び方</div><div>${esc(state.mode === 'main' ? '主将から考える' : state.mode === 'purpose' ? '目的から考える' : '型を直接選ぶ')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">主将</div><div>${esc(state.mainGeneral?.displayName || state.mainGeneral?.name || '指定なし')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">目的</div><div>${esc(purpose()?.purposeName || '指定なし')}</div></div><div class="hte-confirm-row"><div class="hte-confirm-label">型</div><div>${esc(typeRule()?.typeName || '未選択')}</div></div></div><div class="hte-note" style="margin-top:10px">内容を確認し、「選択を保存」を押してください。</div></div>`;
  }
